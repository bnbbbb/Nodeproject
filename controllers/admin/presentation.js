...
const { deleteImage } = require('../../utils/s3Utils');
...
const editPresentation = async (req, res, next) => {
  ...
  try {
    ...
    if (req.file) {
      deleteImage(verificationResult.img);
...
const deletePresentation = async (req, res, next) => {
  ...
  try {
    ...
    deleteImage(presentation.img);

    const deleteQuery = `DELETE FROM presentations WHERE id = ?`;
